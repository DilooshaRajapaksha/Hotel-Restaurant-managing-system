...
function AddStaffModal({ onSave, onClose }) {
  const [form,   setForm]   = useState({ sName: "", email: "", contactNumber: "", passwordHash: "", role: "DELIVERY_STAFF" });
  ...
  const handleSave = async () => {
    ...
    if (!form.passwordHash.trim())  { setError("Password is required.");       return; }
    setSaving(true);
    try {
      const res = await api.post(`/api/delivery/staff`, form);
      onSave(res.data);
    } catch (e) {
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div style={{ background: "#fff", borderRadius: 16, padding: 28, maxWidth: 440, width: "100%", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {[
            { label: "Full Name",       key: "sName",          type: "text",     placeholder: "e.g. Kasun Perera"      },
            { label: "Email",           key: "email",          type: "email",    placeholder: "e.g. kasun@hotel.lk"    },
            { label: "Contact Number",  key: "contactNumber",  type: "text",     placeholder: "e.g. 0771234567"        },
            { label: "Password",        key: "passwordHash",   type: "password", placeholder: "Set initial password"   },
          ].map(({ label, key, type, placeholder }) => (
            <div key={key} style={{ display: "flex", flexDirection: "column", gap: 5 }}>
              <label style={{ fontSize: 13, fontWeight: 600, color: "#374151" }}>{label} <span style={{ color: "#EF4444" }}>*</span></label>
...
export default function AdminDeliveryStaffPage() {
  ...
  const loadStaff = async () => {
    setLoading(true);
    try {
      const res  = await api.get(`/api/delivery/staff`);
      const list = res.data || [];
      setStaff(list);
      const countMap = {};
      await Promise.all(list.map(async s => {
        try {
          const cr = await api.get(`/api/delivery/staff/${s.staffId}/assignments/count`);
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await api.delete(`/api/delivery/staff/${deleteId}`);
      setStaff(prev => prev.filter(s => s.staffId !== deleteId));
      showToast("Staff member removed.");
    } catch (e) {
      showToast(e.response?.data || "Failed to delete.");
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .view-btn:hover { background: linear-gradient(135deg,${GOLD},#8B6914) !important; color: #fff !important; border-color: transparent !important; }
      ...
      {deleteId && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: 28, maxWidth: 380, width: "90%", textAlign: "center", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
            ...
            <p style={{ fontSize: 13, color: "#6B7280", marginBottom: 20 }}>All their order assignments will be cleared first.</p>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0, overflow: "auto" }}>
          ...
          <div style={{ background: "#fff", borderBottom: "1px solid #E5E7EB", padding: "0 32px", height: 64, display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 10 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13 }}>
              ...
              <span style={{ color: "#111827", fontWeight: 600 }}>Delivery Staff</span>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div style={{ fontSize: 11, color: "#9CA3AF" }}>administrator@goldenstar.lk</div>
            </div>
          </div>
          ...
          <div style={{ padding: "28px 32px", flex: 1 }}>
            ...
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 24 }}>
              <div>
                ...
                <p style={{ fontSize: 14, color: "#6B7280" }}>Manage delivery team members and their order assignments.</p>
              </div>
                          ...
                          <tr key={s.staffId} className="sr" style={{ background: i % 2 === 0 ? "#fff" : "#FAFAFA", borderBottom: "1px solid #F9FAFB" }}>
                            ...
                            <td style={{ padding: "14px 20px" }}>
                              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                                ...
                                <span onClick={() => navigate(`/admin/delivery/staff/${s.staffId}`)}
                            ...
                            <td style={{ padding: "14px 20px" }}>
                              <span style={{ display: "inline-flex", alignItems: "center", gap: 5, background: assignCount > 0 ? "#D1FAE5" : "#F3F4F6", color: assignCount > 0 ? "#065F46" : "#6B7280", fontSize: 12, fontWeight: 700, padding: "4px 10px", borderRadius: 20 }}>
                            ...
                            <td style={{ padding: "14px 20px", whiteSpace: "nowrap" }}>
                              <button className="view-btn"
                                onClick={() => navigate(`/admin/delivery/staff/${s.staffId}`)}
